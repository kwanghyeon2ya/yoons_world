	...
	<script>
		$(document).ready(function(){
			...
			function insertForm(){
				...
				var phone1_confirm = RegExp(/^01[016789]{1}$/);
				...
				console.log("userPw값 확인 : "+document.getElementsByName("userPw")[0].value);
		        ...
		        if($("#phone3").val() == ""){
		       	...
		       	if(!phone3_confirm.test(extension)){
		       		alert("내선번호는 4자리의 숫자만 입력할 수 있습니다.");
		        	$("#extension").focus();
		        	return false;
		       	}
		       	if($("#extension").val() == ""){
		...
		function DuplicatedIdCheck(){
			...
			$.ajax({
				url : '/member/duplicatedIdCheck?userId='+$("#userId").val(),
				type : 'GET',
				async : true,
				success : function(data){
					switch(Number(data)){
