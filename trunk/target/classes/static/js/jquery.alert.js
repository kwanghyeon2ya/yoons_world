...
(function($) {
	...
	$.fn.choose = function(msg,op) {$(this).queue(function(){$.alert(msg,op);});};
    ...
    var con = {
        ...
        return $('<div id="'+con.idx+'" class="popup">').append(
                ...
                ).append('<a href="#" class="exit-btn">X</a>')
    ...
    ,button = {
        ...
        prompt:function(op){return $("<ul "+(op.buttonReverse?"class='button-reverse'":"")+">")
                .append($('<li>').append($('<a href="#" class="close">'+op.cancelButton+'</a>')))
                .append($('<li>').append($('<a href="#" class="confirm">'+op.confirmButton+'</a>')))},
        choose:function(op){return $("<ul "+(op.buttonReverse?"class='button-reverse'":"")+">")
        		.append($('<li class="'+op.hidCancel+'">').append($('<a href="#" class="close blue">'+op.cancelButton+'</a>')))
                .append($('<li>').append($('<a href="#" class="exit">'+op.exitButton+'</a>')))
                .append($('<li class="'+op.hidConfirm+'">').append($('<a href="#" class="confirm">'+op.confirmButton+'</a>')))},
    },event = {
    ...
    $.choose = function(msg,op) {
        ...
        event.creation(function() {
            $("#" + con.idx).addClass('confirm');
            $("#" + con.idx).find('#' + con.btn).append(button.choose(op));
            $("#" + con.idx).find('#' + con.tt).html(op.title);
            $("#" + con.idx).find('#' + con.tb).html(msg);
            $("#" + con.idx).find('#' + con.em).html(op.em);
            $("#" + con.idx).find('#' + con.btn).find('.confirm').unbind('click.confirm').bind('click.confirm', function (e) {
            ...
            $("#" + con.idx).find('#' + con.btn).find('.exit').unbind('click.exit').bind('click.exit', function (e) {
                e.preventDefault()
                event.close();
            })
            $("#" + con.idx).find('.exit-btn').bind('click',function(e){
				e.preventDefault()
				event.close();
			});
